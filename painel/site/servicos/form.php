<?php
    ...
    if($_POST['acao'] == 'salvar'){
      ...
      if($_POST['base64'] and $_POST['imagem_tipo'] and $_POST['imagem_nome']){
        ...
        if($_POST['imagem']) unlink("../volume/servicos/{$_POST['imagem']}");
        ...
        if(!is_dir("../volume/servicos")) mkdir("../volume/servicos");
        if(file_put_contents("../volume/servicos/".$nome, $img)){
          $dados['imagem'] = $nome;
        }
      }
      ...
      if($_POST['codigo']){
        $query = "UPDATE servicos set ".implode(", ",$campos)." WHERE codigo = '{$_POST['codigo']}'";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }else{
        $query = "INSERT INTO servicos set ".implode(", ",$campos)."";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }
    ...
    if($_POST['cod']){
      $query = "select * from servicos where codigo = '{$_POST['cod']}'";
