<?php
    ...
    if($_POST['acao'] == 'salvar'){
      ...
      if($_POST['codigo']){
        $query = "UPDATE depoimentos set ".implode(", ",$campos)." WHERE codigo = '{$_POST['codigo']}'";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }else{
        $query = "INSERT INTO depoimentos set ".implode(", ",$campos)."";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }
    ...
    if($_POST['cod']){
      $query = "select * from depoimentos where codigo = '{$_POST['cod']}'";
...
<h3 class="titulo<?=$md5?>">Gerenciamento de Depoimentos</h3>
    ...
    <form id="acaoMenu">
      ...
      <div class="mb-3 form-floating">
        ...
        <div class="mb-3 form-check">
          <input type="radio" <?=(($d->avaliacao == $i)?'checked':false)?> class="form-check-input" value="<?=$i?>" name="avaliacao" id="avaliacao<?=$i?>">
