<?php
    ...
    if($_POST['acao'] == 'salvar'){
      ...
      if($_POST['base64'] and $_POST['imagem_tipo'] and $_POST['imagem_nome']){
        ...
        if($_POST['imagem']) unlink("../volume/time/{$_POST['imagem']}");
      ...
      if($_POST['codigo']){
        $query = "UPDATE time set ".implode(", ",$campos)." WHERE codigo = '{$_POST['codigo']}'";
...
<h3 class="titulo<?=$md5?>">Gerenciamento do Time da Empresa</h3>

    <form id="acaoMenu">

      <div class="form-floating mb-3">
        <input type="text" class="form-control" id="nome" name="nome" placeholder="Título do Banner" value="<?=$d->nome?>">
        <label for="nome">Nome</label>
        <div class="form-text">Digite o seu nome.</div>
      </div>

      <div class="form-floating mb-3">
        <input type="text" class="form-control" id="cargo" name="cargo" placeholder="Nome do cargo na empresa" value="<?=$d->cargo?>">
        <label for="cargo">Cargo</label>
        <div class="form-text">Digite o nome do seu cargo na empresa.</div>
      </div>

      <div showImage class="form-floating" style="display:<?=(($d->imagem)?'block':'none')?>">
        <img src="<?=$localPainel?>site/volume/time/<?=$d->imagem?>" class="img-fluid mt-3 mb-3" alt="" />
      </div>
