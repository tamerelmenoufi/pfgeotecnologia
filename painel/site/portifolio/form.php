<?php
    ...
    if($_POST['acao'] == 'salvar'){
      ...
      if($_POST['codigo']){
        $query = "UPDATE portifolio set ".implode(", ",$campos)." WHERE codigo = '{$_POST['codigo']}'";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }else{
        $query = "INSERT INTO portifolio set ".implode(", ",$campos)."";
        mysqli_query($con, $query);
        $acao = mysqli_affected_rows($con);
      }
    ...
    if($_POST['cod']){
      $query = "select * from portifolio where codigo = '{$_POST['cod']}'";
...
<h3 class="titulo<?=$md5?>">Gerenciamento de Serviços</h3>
    ...
    <form id="acaoMenu">
      ...
      <div showImage class="form-floating" style="display:<?=(($d->imagem)?'block':'none')?>">
        <img src="<?=$localPainel?>site/volume/portifolio/<?=$d->imagem?>" class="img-fluid mt-3 mb-3" alt="" />
      </div>
      ...
      <input type="hidden" id="categoria" name="categoria" value="<?=(($d->categoria)?:$_SESSION['categoria_cod'])?>" >
